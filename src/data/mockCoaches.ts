// Mock coach data for development and testing
export interface Coach {
  id: string;
  name: string;
  personality: string;
  description: string;
  avatar: string; // Can be emoji or local image path
  imageUrl?: any; // Local require() image
  specialties: string[];
  available: boolean;
}
...
export const getAvailableCoaches = (): Coach[] => {
  return mockCoaches.filter(coach => coach.available);
};
